...
class LikeTest extends PantherTestCase
{
    ...
    public function testLikePostWorks(): void
    {
        ...
        $form = $crawler->filter('form[name=login]')->form([
            '_username' => 'louis@pinoit.com',
            '_password' => 'password'
        ]);

        $client->submit($form);

        $element = $client->getWebDriver()->findElement(WebDriverBy::cssSelector(self::GLOBAL_ELEMENT_SELECTOR));

        $filledSvg = $client->getWebDriver()->findElement(WebDriverBy::cssSelector(self::FILLED_SVG_SELECTOR));
